...
import hr.model.bussinessArea.Address;
import hr.model.bussinessArea.AddressData;
import hr.model.bussinessArea.BusinessArea;
import hr.model.bussinessArea.BusinessAreaRequest;
import hr.model.bussinessArea.RequestHeader;
...
public class CertificateTest{	
	...
	private static final Logger LOG = LoggerFactory.getLogger(CertificateTest.class);
...
*/
	...
	public void test(){
		...
		Fiscalization fiskal = new Fiscalization("", "naziv_cert", "password");
		...
		/* Popuna objekata potrebnih za prijavu poslovnog prostora */
				 ...
				 */
				businessArea.setNoteOfBusinessArea("PROD1");
 ...
 ---------------------------------------------------------------------------------------------------------------------------------------- */
			...
			//	bill.setNoteOfOrder("P");
			  ...
			  try{
				  ...
				  LOG.info(fiskal.writeSoap(fiskal.sendSoap(fiskal, billRequest)));
				//  LOG.info(fiskal.writeSoap(new SignVerify().signSoap(new CreateXmls().createSoapMessage(new CreateXmls().createXmlForRequest(billRequest)), fiskal)));
				  //  LOG.info(fiskal.sendEchoMessage(fiskal));
			}catch(Exception e){
