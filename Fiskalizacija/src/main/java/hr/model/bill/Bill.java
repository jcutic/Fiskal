...
public class Bill{
	...
	public String securityCode(String oib, String date, String brBill, String noteBussArea, String noteExcangeDevice, String totalValue, Fiscalization fiskal) {
		...
		try{
			FileInputStream fileInputStream = new FileInputStream(fiskal.getPathOfJKSCert() + 
			...
			keyStore.load( fileInputStream, fiskal.getPasswdOfJKSCert().toCharArray() );
			
			Signature biljeznik = Signature.getInstance("SHA1withRSA");
			biljeznik.initSign((PrivateKey)keyStore.getKey( fiskal.getAliasForPairJKSCert(), fiskal.getPasswdOfJKSCert().toCharArray()));
			biljeznik.update(tempResult.getBytes());
			zki = DigestUtils.md5Hex(biljeznik.sign());
		}catch(Exception e){
			String message = "Došlo je do pogreške kod dohvaæanja ZKI-a!";
			LOG.error(message, e);
