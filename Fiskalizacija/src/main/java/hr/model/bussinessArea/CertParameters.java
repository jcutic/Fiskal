...
public class CertParameters{
	...
	private static final Logger LOG = LoggerFactory.getLogger(CertParameters.class);
	...
	public CertParameters(File paramFile, String password){
		this(paramFile.getPath(), paramFile.getName(), password);
	}
	...
	public String getOIBFromCert(KeyStore keystore, String alias) throws KeyStoreException{
			...
			LOG.info("Podatci iz SubjectDN-a: " + subjectDn);
			...
			if (m.find()){
				oib = m.group(1);
				LOG.info("OIB iz certifikata je: " + oib);
			}
			return oib;
	}
