...
public class SignVerify{
	...
	private static final Logger LOG = LoggerFactory.getLogger(SignVerify.class);
	 ...
	 private static final String PART_SHEMA_NAMESPACE = " ../schema/FiskalizacijaSchema.xsd";
	  ...
	  */
	   public SOAPMessage signSoap(SOAPMessage soapMessage, CertParameters certParameters){
	        ...
	        try{
	    			...
	    			String idForSign = BUSINESS_AREA_ID;
	    			if(new String(out.toByteArray()).contains(BILL_ID)){
	    				idForSign = BILL_ID;
	    				
	    				/* Dodavanje potrebnih namespaceova u XML-u koja se nalazi u Bodyu  SOAP poruke */
	    				@SuppressWarnings("rawtypes")
						Iterator i = soapMessage.getSOAPBody().getChildElements() ;
	    				SOAPBodyElement soapBodyElement = (SOAPBodyElement) i.next();
	    				soapBodyElement.addAttribute(new QName(CreateXmls.XSI_NAMESPACE, CreateXmls.SCHEMA_LOCATION, CreateXmls.XSI), CreateXmls.TNS_NAMESPACE + PART_SHEMA_NAMESPACE);
	    		
	    			}
	    			
	    			/* Pronalazak elemeta s id prema koje se vrši potpis unutar soap poruke */
	    			nodeList = (NodeList) xpath.compile(String.format(PATTERN_FOR_GET_ID, idForSign))
				    ...
				    if (nodeList.getLength() == 0){
				        throw new FiskalizacijaException(NO_ID_FOR_SIGN);
				    }
				...
				/* Elementi koji æe biti dodani u XML nakon potpisa */
				transforms = new ArrayList<Transform>(){
					...
					/* Uèitavanje privatnog kljuèa za potpisivanje i javnog za spremanje u XML */
	    ...
	    Boolean verifyMessage(SOAPMessage soapMessage){
	        ...
	        try {
				...
				if(nodeList.getLength() == 0){
				    throw new FiskalizacijaException(NO_TAG_SIGNATURE);
				}
